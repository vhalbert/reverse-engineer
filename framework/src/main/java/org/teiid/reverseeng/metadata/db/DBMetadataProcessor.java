...
public final class DBMetadataProcessor implements MetadataProcessor {
	...
	private void processColumns(Table dbTable, ResultSet columns)
			throws SQLException {
	 ...
	 * @throws SQLException
	 */
	private void addColumn(ResultSet columns, Table dbtable, int rsColumns, int order) throws SQLException {

		String columnName = columns.getString(4);
		Column column = dbtable.createColumn(columnName);
...
//		return column;
	...
	private void setPrimaryKeys(DatabaseMetaData metadata, Table dbTable, String catalogName, String schemaName)
		throws SQLException {
	...
	private void setIndexes(DatabaseMetaData metadata, Table dbTable, String catalogName, String schemaName) throws SQLException {

		// only pull unique keys so that an attribute can be flagged as required 
...
//					LogManager.logDetail(LogConstants.CTX_CONNECTOR, "Skipping the import of non-simple index", indexInfo.getString(6)); //$NON-NLS-1$
			...
			boolean isUnique = indexInfo.getBoolean(4);
			col.setIsRequired(isUnique);
